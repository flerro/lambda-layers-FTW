...
aws s3 cp build/layer.zip s3://lambda-layer-ftw-deploy/
...
echo "Creating stack..."
sam deploy --capabilities CAPABILITY_IAM --template-file deploy.yml --stack-name layer-ftw-node --s3-bucket $BUCKET --region $REGION
